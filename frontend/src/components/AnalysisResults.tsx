...
score?: number; // Used for AI_ANALYSIS
confidence_score?: number; // Used for legacy/other detectors
label?: string; // Used for AI_ANALYSIS
entities?: any[]; // Used for PII_DETECTION
explanation?: string;
flags?: string[];
short_explanation?: string;
risk_label?: string; // Added to represent the risk label for each detector finding
}
...
results?: DetectorFinding[]; // This holds the LIST of results from the router's output
risk_label?: string;
risk_score?: number;
}
...
// Use the overall risk_label for the top card styling
const finalRiskLabel = result?.risk_label || (result.results?.[0]?.label) || 'UNKNOWN';

// finalResultObject now holds the array of individual detector findings (AI, PII, etc.)
...
const getDetectorIcon = (type?: string) => {
    switch (type?.toUpperCase()) {
        case 'PII_DETECTION': return <Fingerprint className="w-5 h-5" />;
        case 'AI_ANALYSIS': 
        case 'PDF_TEXT_AI': 
...
return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* 1. Top Summary Card - **This is the ONLY place metadata should be explicitly rendered** */}
            ...
            {finalResultObject.length > 0 ? (
                finalResultObject.map((detector: DetectorFinding, idx: number) => {
                    ...
                    if (detector.type?.toUpperCase() === 'PII_DETECTION') {
                        ...
                        // FIX: Set confidence to 1.0 if PII entities were found, 0.0 otherwise.
                        confidence = entitiesFound ? 1.0 : 0.0; 
                        
                        // FIX: Cleaned up explanation to show PII status only once
                    ...
                    return (
                        <div key={idx} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-shadow duration-200">
                            ...
                            {/* Show flags/entities for PII */}
                            {detector.type?.toUpperCase() === 'PII_DETECTION' && Array.isArray(detector.entities) && detector.entities.length > 0 && (
                                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                                    <h4 className="font-semibold text-sm mb-2 text-gray-900 dark:text-white">Detected PII Entities ({detector.entities.length})</h4>
